...
async function renderQuranReader(){
 ...
 app.innerHTML=`<section class="readerTop quranReaderTop"><button class="readerBack" id="quranBack">←</button><div><small>KUR’ÂN-I KERÎM · ${meta.id}/114</small><b>${esc(meta.turkish)} · ${esc(meta.arabic)}</b></div><div class="readerTools"><button id="quranFocus" class="${state.focusMode?'active':''}" aria-pressed="${state.focusMode?'true':'false'}">${state.focusMode?'Odaktan çık':'Odak'}</button><button id="quranFontDown">A−</button><button id="quranFontUp">A+</button></div></section>
