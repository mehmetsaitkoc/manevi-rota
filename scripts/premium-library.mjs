...
assert.equal(kurandanAyetler.source?.normalizationVersion,2);
assert.match(kurandanAyetler.pages[0].text,/Kur’an onun \(hem semavî/i);
assert.match(kurandanAyetler.pages[0].text,/Mehmet Akif de aynı yolu tutan/i);
assert.doesNotMatch(kurandanAyetler.pages[0].text,/Kur W onum|Kur'ao|Mehmet Akilde|\bKur an\b/);
